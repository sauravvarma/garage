...
The exception is the design comp itself (Pencil frames). Those *do* get replaced — but the artifact index in `DESIGN-TAXONOMY.md` records the lifecycle, so the history of that comp is recoverable even if the file isn't. `DESIGN-TAXONOMY.md` is the canonical definition of that lifecycle: a six-state machine (`draft → {concept | variant} → {anchor | final} | deprecated`) across three separated axes (lifecycle on the frame's bracket, origin on the enclosing group, sync state on the index row). Promotion between states is **atomic** (winner + losing siblings + prior terminal + index rows all change in one operation), and four invariants (one `[final]` and one `[anchor]` per artifact; candidates only inside dated groups; terminals don't transition back except via an audit-trailed Mode 5 Pivot) are enforced by every skill that reads the file — `design-agent`, `code-agent`, and `visual-qa` each block on a violation. This is what makes "the design comp gets replaced" safe: replacement is a tracked state transition, not a silent overwrite.
